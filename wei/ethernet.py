...
#Predefinimos levelInitialized a False
...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
	...
	#Si la trama no es para nosotros, descartamos
	if (dest != macAddress and dest != broadcastAddr):
		return
...
def startEthernetLevel(interface:str) -> int:
	...
	#Comprobamos si el enlace esta creado
	...
	#Marcamos levelInitialized como true
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
	...
	trama = bytearray()
	trama[0:6] = dstMac
	trama[6:12] = macAddress
	trama[12:14] = etherType.to_bytes(2, 'big')
	
	#Añadimos datos (asegurando que haya al menos 60 bytes)
	trama[14:len(data)] = data	
	if (len(trama) < ETH_FRAME_MIN):
		trama[len(trama):] = bytearray(ETH_FRAME_MIN - len(trama)) 

	#Enviamos trama
	pcap_inject(handle, bytes(trama), len(trama))
